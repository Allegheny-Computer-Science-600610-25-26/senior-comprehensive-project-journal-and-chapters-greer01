...
This combination of data analysis and conversational AI separates CaddySense from traditional systems. As shown in Figure 1, the system is made up of both an engine that is analytical, and a communicational interface, which is designed in order to make performance data meaningful and attainable. When these components are put together, they form a data flow that combines analytical precision with communication intelligence. The overall system structure, shown in Figure 1 (System Architecture Diagram) below, shows this data flow:
...
A model that is prototyped in Python, then generates visual summaries that show the user's performance trends. The visuals shown in Figures 2 and 3, show histograms, scatter plots, and radar charts, and their purpose is to act as another layer to help the user understand their strengths and weaknesses. These visuals are a good way to display multiple areas of reasoning about a golfer’s game.

![Driving distance distribution across a synthesized dataset of golfers. This histogram shows the wide performance measures of driving distance, which allows for CaddySense's need for AI coaching, rather than traditional non-personalized coaching.](images/HistogramCP.png){width=75%}

![Relationship between driving distance to greens in regulation percentage. This scattered pattern shows how longer drivers of the golf ball do not always score better on approach shots. This shows how the system needs multiple metrics instead of relying on a few.](images/ScatterplotCP.png){width=75%}
...
This chapter introduced the motivation, project goals, and ethical background of CaddySense, a tool using an AI-powered golf performance assistant that can transform raw golf player data into more meaningful, personalized advice to get better at the game of golf. This chapter highlights how, even though there is an upcoming expansion of sports analytics, current systems often stop at measurement rather than interpretation. To prevent this, CaddySense is in the position of an interpretable and conversational-based AI system that can fill this gap between data collection and insight. Helping golfers actually understand not only what happened when it comes to their game, but also why it occurred and how exactly they can improve.
